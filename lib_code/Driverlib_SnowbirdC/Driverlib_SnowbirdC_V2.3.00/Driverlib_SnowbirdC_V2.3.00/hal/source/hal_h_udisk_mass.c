...
T_U8 msc_read_format_capacity(T_U8 Lun,T_U8* buf,T_U8* status)
{
    ...
    drv_print("read f_capacity", 0, AK_TRUE);
    //set inquiry cmd
...
T_U8 msc_read_capacity(T_U8 Lun,T_U8* buf,T_U8* status)
{
    ...
    drv_print("read capacity", 0, AK_TRUE);
    //set inquiry cmd
   ...
   //fill urb struct
    ...
    while (1)
    {
        ...
        if(usb_bus_wait_completion(hURB) < 0)
        {
            if (retry++ < 1)
            {
                drv_print("reset recovery", 0, AK_TRUE);
...
static T_BOOL msc_status_stage(T_USB_CSW_PKT* csw)
{
    ...
    else
    {
        ...
        {
            drv_print("csw invalid", 0, AK_TRUE);
            goto  reset_recovery;
        }
        if (csw->bCSWStatus != 0  && csw->bCSWStatus != 1)
        {
            drv_print("csw phase error", 0, AK_TRUE);
            goto  reset_recovery;
        }
        return AK_TRUE;
    }
