...
#define DEVICE_NAME             "CAMERA:GC0308"
...
#define GC0308_CAMERA_MCLK      300 //单位为100kHz
...
{
    const T_U8 PowerDownSet[] = 
    {
        0x1a,0x2b,//工作时钟分频比与占空比
        END_FLAG, END_FLAG
    };
    camera_set_param(PowerDownSet);
    gpio_set_pin_level(GPIO_CAMERA_CHIP_ENABLE, !CAM_EN_LEVEL);
    //gpio_set_pin_level(GPIO_CAMERA_AVDD, !gpio_pin_get_ActiveLevel(GPIO_CAMERA_AVDD));    
...
/**
 * @brief Set camera mode to specify image quality, VGA/CIF etc 
 * @author xia_wenting 
 * @date 2011-01-11
...
static T_BOOL cam_gc0308_set_RawRGB(T_U16 DstWidth,T_U16 DstHeight)
{
    T_U8 HighBit;
    T_U16 xStart;
    T_U16 yStart;
	const T_CHR Format[][3] = {"BG","GB","GR","RG"};
	
    T_U8 Camera_crop_window_table[] =
    {
        //使能crop window
        ...
        //不使能子采样
        0xfe,0x01,//Page1 REG
        0x53,0x02,// opclk gated disable in subsample
        0x55,0x02,//subsample_extend_opclk = 0
        0xfe,0x00,//Page0 REG
        END_FLAG, END_FLAG
    };
	
	
    if(DstWidth > 648|| DstHeight>488)
        return AK_FALSE;
	
    xStart = (648 - DstWidth)/2;
    yStart = (488 - DstHeight)/2;
    HighBit = (xStart>>8)&0x07;//[2:0] x0
    HighBit |= (((yStart>>8)&0x3)<<4);//[5:4] y0
	
    Camera_crop_window_table[1] |= HighBit;
    Camera_crop_window_table[3] = (T_U8)(yStart&0xff);
    Camera_crop_window_table[5] = (T_U8)(xStart&0xff);
    Camera_crop_window_table[7] = (T_U8)((DstHeight>>8)&0x1);
    Camera_crop_window_table[9] = (T_U8)((DstHeight)&0xFF);
    Camera_crop_window_table[11] = (T_U8)((DstWidth>>8)&0x3);
    Camera_crop_window_table[13] = (T_U8)((DstWidth)&0xFF);

    camera_setup(RAWRGB_TAB);
    camera_setup(Camera_crop_window_table);

	drv_print("RAW_RGB:first pixel:", 0, 0);
    drv_print((Format[((yStart%2)<<1) + xStart%2]), 0, 1);
    return AK_TRUE;
}
...
static T_CAMERA_FUNCTION_HANDLER gc0308_function_handler = 
{
    ...
    cam_gc0308_get_type,
    cam_gc0308_set_RawRGB
};

int camera_gc0308_reg(void)
{
