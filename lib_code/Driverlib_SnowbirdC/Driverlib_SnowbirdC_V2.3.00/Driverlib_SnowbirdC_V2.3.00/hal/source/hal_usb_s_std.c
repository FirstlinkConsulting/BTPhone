...
T_BOOL usb_slave_std_clear_feature(T_CONTROL_TRANS *pTrans)
{
    ...
    //only handle in setup and status stage
    if((pTrans->stage != CTRL_STAGE_SETUP) && (pTrans->stage != CTRL_STAGE_STATUS))
        return AK_TRUE;
    ...
    if(!m_hard_stall)
    {
        //we clear stall in setup stag, in case that code/data abort may bring delay in status stag
        if(pTrans->stage == CTRL_STAGE_SETUP)
            usb_slave_ep_clr_stall(ep_num);

        //and then we change status in status stage
        if(pTrans->stage == CTRL_STAGE_STATUS)
            usb_slave_set_ep_status(ep_num, AK_FALSE);
    }
    return AK_TRUE;
}
...
T_BOOL usb_slave_std_get_descriptor(T_CONTROL_TRANS *pTrans)
{
    ...
    switch( des_type )
    {
        ...
        case REPORT_DESC_TYPE:
            if (Usb_Slave_Standard.usb_get_hid_report_descriptor != AK_NULL)
            {
                data = Usb_Slave_Standard.usb_get_hid_report_descriptor(&cnt);
            }
            else
            {
                return AK_FALSE;
