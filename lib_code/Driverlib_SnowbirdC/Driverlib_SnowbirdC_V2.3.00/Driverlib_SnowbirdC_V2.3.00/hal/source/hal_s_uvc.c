...
extern volatile  T_BOOL  g_bUvcSending;

static T_pUVC_DEV s_pUvcDev = AK_NULL;
static T_U8 s_ConfigData[500] = {0};

static T_VOID uvc_dev_msg_proc(T_U32* pMsg,T_U32 len);
...
const static T_UVC_SELECTOR_UNIT_DESCRIPTOR s_UvcSelectUnitDesc =   
{
    ...
    0x01,                                                           ///< Input 1 of this unit is connected to unit ID 0x01 – the CMOS sensor.
    0x02,                                                           ///<  Input 2 of this unit is connected to unit ID 0x02 – the composite connector.
    0x00                                                            ///<  Unused
};
...
T_VOID uvc_get_frame_res(T_pUVC_FRAME_RES pFrameRes,T_U32 FrameId)
{
    ...
    {
        drv_print("uvc_get_frame_res():param error!\n", 0, AK_FALSE);
        return;
    }
...
T_BOOL uvc_init_desc()
{
    ...
    {
        drv_print("all config desc size error", cnt, AK_TRUE);
        return AK_FALSE;
    }
    drv_print("init all config desc ok:",cnt, AK_TRUE);
    return AK_TRUE;
}
...
T_BOOL uvc_init(T_U32 mode,T_U8 format)
{
    ...
    {
        drv_print("s_pUvcDev,alloc failed\n", 0, AK_TRUE);
        return AK_FALSE;
    }  
...
#endif
    //map message
    DrvModule_Map_Message(DRV_MODULE_UVC, UVC_DEV_MSG, uvc_dev_msg_proc);
...
T_U32 uvc_payload(T_U8* pPayload, T_U8* pData, T_U32 dwSize)
{
    ...
    {
        drv_print("error:payload buffer or data buffer null\n", 0, AK_FALSE);
        return 0;
    }
    i = dwSize;
    j = 0;
    ...
    while (i > 0)
    {
        //in order to support no-remap system, we do some adjust here,
        //copy data first and then set payload head,
        //so we can use the same buffer to complete payload process
        len = s_VideoProbeCommitControls.dwMaxPayloadTransferSize - 2;
        if(i < len) len = i;

        for(k = len-1; k >= 0; k--)
            pPayload[j + 2 + k] = pTmp[k];
        ...
        else
        {
            ...
            /** if the payload size is multipy of 512, there are error in host size,
                so we do add a empty payload in that condition.
            */

            if(((j+i+2) & 0x3f) == 0)
            {
                pPayload[j+i+2] = 0x02;
                pPayload[j+i+3] = 0x80|fid|2;    
...
T_BOOL uvc_short_payload(T_U8* pPayload, T_U8* pData, T_U32 dwSize, T_eUVC_PAYLOAD flag)
{
    ...
    if (pPayload == AK_NULL || pData == AK_NULL)
    {
        drv_print("error:payload buffer or data buffer null",0,AK_TRUE);
        return AK_FALSE;
    }
    i = dwSize;
    j = 0;
    ...
    if  (UVC_PAYLOAD_START == flag)
    {
        memcpy(&pPayload[j + 2], pTmp, dwSize);
    }
    else
    {
        memcpy(&pPayload[j + 2], pTmp, i);
...
static T_VOID uvc_send_finish()
{
    ...
    msg.ucMsgId = MSG_FRAME_SENT;
    msg.ucParam2 = 0;
    msg.ulParam3 = 0;
    msg.ulParam4 = 0;
    DrvModule_Send_Message(DRV_MODULE_UVC, UVC_DEV_MSG, (T_U32*)&msg);
}
...
static T_BOOL uvc_class_callback(T_CONTROL_TRANS *pTrans)
{
    ...
    if (RECIPIENT_INTERFACE == (pTrans->dev_req.bmRequestType & 0x1f))
    {
        switch (pTrans->stage)
        {
            case CTRL_STAGE_SETUP: 
                ...
                {
                    drv_print("GET: vc ctrl unsupported,stall ep0\n", 0, AK_FALSE);
                    ret = AK_FALSE;
                }
                else
                {   
                            ...
                            switch (LOBYTE(pTrans->dev_req.wIndex))
                            {
                                case UVC_VCI_ID :
                                    switch (HIBYTE(pTrans->dev_req.wIndex))
                                    {
                                        case UVC_PU_ID :       
                                            ...
                                            break;         
                                        default:
                                            drv_print("Unsupport GET_CUR:Unit\n", 0, AK_FALSE); 
                                            //ep0 will stall
                                            ret = AK_FALSE;
                                            break;
                                    }
                                ...
                                case UVC_VSI_ID :
                                    switch (HIBYTE(pTrans->dev_req.wValue))
                                    {
                                        ...
                                        default:
                                            drv_print("GET_CUR:invalid ctrl selector\n", 0, AK_FALSE); 
                                            //ep0 will stall
                                            ret = AK_FALSE;
                                            break;
                                    }
                            ...
                            switch (LOBYTE(pTrans->dev_req.wIndex))
                            {
                                case UVC_VCI_ID :
                                    switch (HIBYTE(pTrans->dev_req.wIndex))
                                    {
                                        ...
                                        default:
                                            drv_print("Unsupport GET_MIN:Unit\n", 0, AK_FALSE); 
                                            //ep0 will stall
                                            ret = AK_FALSE;
                                            break;
                                    }
                                ...
                                case UVC_VSI_ID :
                                    switch (HIBYTE(pTrans->dev_req.wValue))
                                    {
                                        ...
                                        default:
                                            drv_print("GET_MIN:invalid ctrl selector\n", 0, AK_FALSE); 
                                            //ep0 will stall
                                            ret = AK_FALSE;
                                            break;
                                    }
                            ...
                            switch (LOBYTE(pTrans->dev_req.wIndex))
                            {
                                case UVC_VCI_ID :
                                    switch (HIBYTE(pTrans->dev_req.wIndex))
                                    {
                                        ...
                                        default:
                                            drv_print("Unsupport GET_MAX:Unit\n", 0, AK_FALSE);
                                            //ep0 will stall
                                            ret = AK_FALSE;
                                            break;
                                    }
                                ...
                                case UVC_VSI_ID:
                                    switch (HIBYTE(pTrans->dev_req.wValue))
                                    {
                                        ...
                                        default:
                                            drv_print("GET_MAX:invalid ctrl selector\n", 0, AK_FALSE);
                                            //ep0 will stall
                                            ret = AK_FALSE;
                                            break;
                                    }
                            ...
                            switch (LOBYTE(pTrans->dev_req.wIndex))
                            {
                                case UVC_VCI_ID :
                                    switch (HIBYTE(pTrans->dev_req.wIndex))
                                    {
                                        ...
                                        default:
                                            drv_print("Unsupport GET_RES:Unit\n", 0, AK_FALSE);
                                            //ep0 will stall
                                            ret = AK_FALSE;
                                            break;
                                    }
                                    break;
                                case UVC_VSI_ID:
                                    drv_print("Unsupport GET_RES: VSI\n", 0, AK_FALSE);
                                    ret = AK_FALSE;
                                    break;
                                ...
                                case UVC_VCI_ID :
                                    switch (HIBYTE(pTrans->dev_req.wIndex))
                                    {
                                        ...
                                        case UVC_SU_ID :
                                            ...
                                            break;
                                        default:
                                            drv_print("Unsupport GET_INFO:Unit\n", 0, AK_FALSE);
                                            //ep0 will stall
                                            ret = AK_FALSE;
                                            break;
                                    }
                            ...
                            switch (LOBYTE(pTrans->dev_req.wIndex))
                            {
                                case UVC_VCI_ID :
                                    switch (HIBYTE(pTrans->dev_req.wIndex))
                                    {
                                        ...
                                        default:
                                            drv_print("Unsupport GET_DEF:Unit\n", 0, AK_FALSE); 
                                            //ep0 will stall
                                            ret = AK_FALSE;
                                            break;
                                    }
                                    break;
                                case UVC_VSI_ID :
                                    drv_print("Unsupport GET_DEF: VSI\n", 0, AK_FALSE); 
                                    ret = AK_FALSE;
                                    break;
                ...
                if (pTrans->dev_req.bRequest == SET_CUR)
                {
                    ...
                    switch (LOBYTE(pTrans->dev_req.wIndex))
                    {
                        ...
                        case UVC_VSI_ID :
                            ...
                            switch (HIBYTE(pTrans->dev_req.wValue))
                            {
                                case VS_PROBE_CONTROL:
                                    if (!g_bUvcSending)
                                    {
                                        ...
                                        drv_print("vs prob, frmID:", s_VideoProbeCommitControls.bFrameIndex, AK_TRUE);
                                    }  
                                    else
                                    {
                                        drv_print("Sorry but already Open!\n", 0, AK_FALSE); 
                                        return ret;
                                    }
                                    break;
                                case VS_COMMIT_CONTROL :
                            ...
                            break;
                        default:
                            drv_print("invalid dev_req.wIndex\n", 0, AK_FALSE); 
                            return AK_FALSE;
                    }
    ...
    {
        drv_print(" not RECIPIENT_INTERFACE ,stall ep0\n", 0, AK_FALSE); 
        ret = AK_FALSE;
    }
    return ret;
