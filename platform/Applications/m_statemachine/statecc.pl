...
my @aIncludes;
my $mstatedir = "../m_statemachine";
my $meventdir = "../../PubInclude";
my $eventfileH = "m_event.h";
my $statefileC = "m_state.c";
my $statefileH = "m_state.h";
my $genStubs = 0;
my $genErrors = 1;
my $genWarnings = 1;
my $fheader;
...
sub printHelp
{
    ...
    print "             -v <0|1>     : if 1, 对video相关代码进行归段\n";
}
...
sub handleArgs
{
    ...
    if (!getopts('f:h:s:e:w:v:', \%hOptions)) 
    {
        printHelp();
        exit;
    }
...
sub createEventFileH
{
    ...
    open(EFILE, ">$meventdir/$eventfile") or die "ERROR: Cannot open $eventfile\n";
    print EFILE "#ifndef _M_EVENT_H\n";
    print EFILE "#define _M_EVENT_H\n";
    print EFILE $fheader;

    print EFILE "#include <vme.h>\n";
    #print EFILE "#include \"m_event_api.h\"\n\n";
...
sub createStateFileH
{
    ...
    open(SFILE, ">$mstatedir/$statefile") or die "ERROR: Cannot open $statefile\n";
...
~;
    ...
    foreach $state (@aStates)
    {        
        #if (not exists $hStates{$state}->{$aStateAttr[0]})
        if ($state eq "preproc")
        {
        		print SFILE "#pragma arm section rodata = \"_bootcode1_\"\n";
        }
        if ($state eq "s_audio_main")
        {
        		print SFILE "#pragma arm section rodata = \"_audioplayer_\"\n";
        }
        if ($state eq "s_cam_videorecord")  
        {
            if ($videoSecAlign eq 1)  
            {
                print SFILE "#pragma arm section rodata = \"_frequentcode_\"\n";   
            }         	
        }
        ...
        if ($state eq "preproc")
        {
        		print SFILE "#pragma arm section rodata\n";
        }
        if ($state eq "s_audio_main")
        {
        		print SFILE "#pragma arm section rodata\n";            
...
~;
    ...
    foreach $state (@aStates)
    {
        ...
        #if ($state eq "preproc" || $state eq "postproc" )
         #if (not exists $hStates{$state}->{$aStateAttr[0]})
        if ($state eq "preproc")
        {
        	print SFILE "#pragma arm section code = \"_bootcode1_\"\n";
      	}
      	if ($state eq "s_audio_main")
      	{
      	    print SFILE "#pragma arm section code = \"_audioplayer_\"\n";
      	}
      	if ($state eq "s_cam_videorecord")  
        {
            if ($videoSecAlign eq 1)  
            {
                print SFILE "#pragma arm section code = \"_frequentcode_\"\n";   
            }         	
        }
        print SFILE "static int $nextfunc(T_EVT_CODE event, M_TRANSTYPE *pTrans)\n{\n";
	...
	@aEvents = @aNormalEvents;
        ...
        #if (not exists $hStates{$state}->{$aStateAttr[0]})
        {
        	print SFILE "#pragma arm section code\n";
      	}
      	if ($state eq "s_audio_main")
      	{
      	   print SFILE "#pragma arm section code\n";
...
~;
    ...
    my @aStates = sort keys %hStates;
    my $state;
    print SFILE "static M_STATESTRUCT* const m_statearray[] = \n{\n";
    for ($id=0; $id<@aStates-1; $id++)
    {
        $state = $hId2State{$id};
        print SFILE " (M_STATESTRUCT* const)&m_$state,\n";
    }
    $state = $hId2State{$id};
    print SFILE "  (M_STATESTRUCT* const)&m_$state\n";
    print SFILE "};\n";
...
~;
    ...
    print SFILE "static _fHandle const m_funcarray[] = \n{\n";
