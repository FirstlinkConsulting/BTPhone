...
#include "arch_init.h"
#include "drv_cfg.h"
#if (DRV_SUPPORT_CAMERA > 0) && (CAMERA_OV9650 > 0)


#define DEVICE_NAME         "CAMERA:OV9650"
...
#define OV9650_CAMERA_MCLK  300//unit:100Khz
...
extern T_U8 sccb_read_data(T_U8 daddr, T_U8 raddr);
extern T_BOOL sccb_write_data(T_U8 daddr, T_U8 raddr, T_U8 *data, T_U32 size);
extern T_VOID sccb_init(T_U32 pin_scl, T_U32 pin_sda);
    ...
    while (1)
    {
        ...
        else
        {
            ...
            {                
                ...
                {
                    drv_print("set parameter error!",0,1);
                    drv_print("reg ",tabParameter[i],0);
                    drv_print(" write:",tabParameter[i + 1],0);
                    drv_print(" read:",temp_value,1);
                    return AK_FALSE;
                }
            }
        }
...
{
    //gpio_set_pin_as_gpio(GPIO_CAMERA_AVDD);//GPIO_CAMERA_AVDD为无效管脚
    //gpio_set_pin_as_gpio(GPIO_CAMERA_ENABLE);
    REG32(REG_SHARE_PIN_CTRL) &= ~(0x03<<10);//gpio_set_pin_as_gpio(GPIO_CAMERA_RESET);
    
    
    //gpio_set_pin_dir(GPIO_CAMERA_ENABLE, GPIO_DIR_OUTPUT);
    //gpio_set_pin_level(GPIO_CAMERA_ENABLE, CAM_EN_LEVEL);    
    //mini_delay(10);
...
{    
    ...
    gpio_set_pin_dir(GPIO_I2C_SCLK, GPIO_DIR_OUTPUT);
    gpio_set_pin_level(GPIO_I2C_SCLK, GPIO_LEVEL_LOW);
...
{    
    ...
    drv_print("i2c SCL:",GPIO_I2C_SCLK,0);
    drv_print("SDA:",GPIO_I2C_SDA,1);

    sccb_init(GPIO_I2C_SCLK, GPIO_I2C_SDA);        //init sccb first here!!
...
static T_VOID cam_ov9650_set_mode(T_CAMERA_MODE mode)
{
    ...
    switch(mode)
    {
        ...
        default:
            ...
            drv_print("set camera mode parameter error!",0,1);
            break;
    }
}
...
static T_VOID cam_ov9650_set_exposure(T_CAMERA_EXPOSURE exposure)
{
    switch(exposure)
    {
        ...
        case EXPOSURE_MIDDLE:
            ...
            break;
        default:
            drv_print("set exposure parameter error!",0,1);
            break;
    }
}
...
static T_VOID cam_ov9650_set_brightness(T_CAMERA_BRIGHTNESS brightness)
{
    switch(brightness)
    {
        ...
        case CAMERA_BRIGHTNESS_6:
            ...
            break;
        default:
            drv_print("set brightness parameter error!",0,1);
            break;
    }
}
...
static T_VOID cam_ov9650_set_contrast(T_CAMERA_CONTRAST contrast)
{
    switch(contrast)
    {
        ...
        case CAMERA_CONTRAST_7:
            ...
            break;
        default:
            drv_print("set contrast parameter error!",0,1);
            break;
    }
}
...
static T_VOID cam_ov9650_set_saturation(T_CAMERA_SATURATION saturation)
{
    switch(saturation)
    {
        ...
        case CAMERA_SATURATION_5:
            ...
            break;
        default:
            drv_print("set saturation parameter error!",0,1);
            break;
    }
}
...
static T_VOID cam_ov9650_set_sharpness(T_CAMERA_SHARPNESS sharpness)
{
    switch(sharpness)
    {
        ...
        case CAMERA_SHARPNESS_6:
            ...
            break;
        default:
            drv_print("set sharpness parameter error!",0,1);
            break;
    }
}
...
static T_VOID cam_ov9650_set_AWB(T_CAMERA_AWB awb)
{
    switch(awb)
    {
        ...
        case AWB_NIGHT:
            ...
            break;
        default:
            drv_print("set AWB mode parameter error!",0,1);
            break;            
    }
}
...
static T_VOID cam_ov9650_set_mirror(T_CAMERA_MIRROR mirror)
{
    switch(mirror)
    {
        ...
        case CAMERA_MIRROR_FLIP:
            ...
            break;                    
        default:
            drv_print("set mirror parameter error!",0,1);
            break;
    }
}
...
static T_VOID cam_ov9650_set_effect(T_CAMERA_EFFECT effect)
{
    switch(effect)
    {
        ...
        case CAMERA_EFFECT_BWN:
            ...
            break;    
        default:
            drv_print("set camer effect parameter error!",0,1);
            break;
    }
}
...
static T_S32 cam_ov9650_set_digital_zoom(T_U32 srcWidth, T_U32 srcHeight)
{
    ...
    drv_print("set window w:",srcWidth,0);
    drv_print(" h:",srcHeight,1);
    ...
    switch(s_ov9650_CurMode)
    {
        ...
        default:
            drv_print("unsupported WINDOWING in mode:", s_ov9650_CurMode,1);
            return 0;
    }
...
static T_VOID cam_ov9650_set_night_mode(T_NIGHT_MODE mode)
{
    switch(mode)
    {
        ...
        case CAMERA_NIGHT_MODE:
            ...
            break;
        default:
            drv_print("set night mode parameter error!",0,1);
            break;
    }
}
...
//    float preview_framerate;    
    ...
    mclk = OV9650_CAMERA_MCLK * 100000;
    ...
    {           
        case CAMERA_MODE_SXGA:
            ...
            else if ((framerate <= 15) && (OV9650_CAMERA_MCLK > 240))
            {
                Reg0x11 = 0x80;         
            }            
            else
            {
                drv_print("ov9650 set SXGA framerate error!",0,1);
                return 0;
            }
        ...
        case CAMERA_MODE_REC:                
            ...
            {
                drv_print("ov9650 set VGA framerate error!",0,1);
                return 0;
            }
        ...
        default:                      
            drv_print("ov9650 unsupport mode:",s_ov9650_CurMode,1);
            return 0;
    }
    ...
    sccb_write_data(CAMERA_SCCB_ADDR, 0xa2, &Reg0xa2, 1);
...
//    akprintf(C3, M_DRVSYS, "pre_gain = 0x%-8x\n", pre_gain);        
    ...
    switch(Cammode)
    {
        ...
        default:
            drv_print("set camera capture mode parameter error!",0,1);
            break;
    }
//    akprintf(C3, M_DRVSYS, "pre_expo = 0x%-8x\n", pre_expo);
...
//    akprintf(C3, M_DRVSYS, "Lines_10ms = 0x%-8x\n", Lines_10ms);
    ...
    if (cap_expo_gain < 1048 * 16)
    {
        ...
        //Capture_gain = 16;
        cap_expo = cap_expo>0?cap_expo:1;
...
{    
    ...
    Mclk = (float)(clk_get_asic() / 1000000) / CAMERA_MCLK_DIV;
    ...
    {
        drv_print("ov9650 unsupport W:",srcWidth,0);
        drv_print(",H:",srcHeight,1);
        return AK_FALSE;
    }
...
#endif  //(DRV_SUPPORT_CAMERA > 0) && (CAMERA_OV9650 > 0)
